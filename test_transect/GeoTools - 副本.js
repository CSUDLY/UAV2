...
(function () {
    ...
    _Line.prototype.unitVector = function () {
        ...
        var f = new _Line(this.p1, QPointF(this.p1.lng + x / len, this.p1.lat + y / len));
        return f;
    }
    ...
    _Line.prototype.setLength = function (distance) {
        ...
        p2 = new _Point(p1.lng + (v.p2.lng - v.p1.lng) * distance, p1.lat + (v.p2.lat - v.p1.lat) * distance);
    }
    ...
    _Line.prototype.setAngle = function (angle) {
        ...
        var dx = qCos(angleR) * l;
        var dy = -qSin(angleR) * l;
    ...
    _Polygon.prototype.calcArea() = function() {
        ...
        for (var i = 0; i < this.points.length; ++i) {
            if (i != 0) {
                ...
                area += this.points[this.points.length - 1].lng * this.points[i].lat - this.points[i].lng * polygon.points[polygon.points.length - 1].lat;
            }
        }
...
function calcPolygonCornor(centerPoint, width, height, mecOrNot) {
