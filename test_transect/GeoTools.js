// F:\Qt\SRC\5.11.1\Src\qtbase\src\corelib\tools
...
// Some class
(function () {
    var _Point = function (lng, lat, alt) {
        this.lat = lat || 0.0;
        this.lng = lng || 0.0;
        this.alt = alt || 0.0;
    };
...
(function () {
    ...
    _Line.prototype.setLength = function (distance) {
        ...
        var dx = this.p2.lng - this.p1.lng;
        var dy = this.p2.lat - this.p1.lat;
        var len = Math.sqrt(dx * dx + dy * dy);
        //this.p2 = new _Point(this.p1.lng + (v.p2.lng - v.p1.lng) * distance, this.p1.lat + (v.p2.lat - v.p1.lat) * distance, 0);
        this.p2 = new _Point(this.p1.lng + dx * distance / len, this.p1.lat + dy * distance / len, 0)
    }
    _Line.prototype.dx = function () {
        return this.p2.lng - this.p1.lng;
    }
    _Line.prototype.dy = function () {
...
(function () {
    ...
    _Polygon.prototype.offset = function (distance) {
        ...
        if (this.points.length > 2) {
            ...
            var rgNedVertices = this.nedPolygon();
            ...
            for (var i = 0; i < rgNedVertices.length; i++) {
                ...
                var offsetEdge = new _Line;//QLineF  
                var originalEdge = new _Line(rgNedVertices[i], rgNedVertices[lastIndex]);

                var workerLine1 = new _Line(originalEdge.p1, originalEdge.p2); // QLinef
                workerLine1.setLength(distance);
                var tmpAngle1 = workerLine1.getAngle();
                ...
                var tmpAngle2 = workerLine2.getAngle();
                workerLine2.setAngle(workerLine2.getAngle() + 90.0);
                offsetEdge.setP2(workerLine2.p2);
            ...
            for (var i = 0; i < rgOffsetEdges.length; i++) {
                var newVertex = new _Point;//QPointF
                var prevIndex = (i == 0 ? rgOffsetEdges.length - 1 : i - 1);
                if (rgOffsetEdges[prevIndex].lineIntersect(rgOffsetEdges[i], null) == 0) {
                    // FIXME: Better error handling?
                    continue;
                }
                newVertex = rgOffsetEdges[prevIndex].lineIntersect(rgOffsetEdges[i], 1);
...
(function () {
    var _ItemCase = function (dj,cmd,frm, p1, p2, p3, p4, p5, p6, p7,ac, ic, tp) {
        this.doJumpId = dj;
        this.command = cmd;
        this.frame = frm;
        this.params = [p1, p2, p3, p4, p5, p6, p7];
        this.autoContinue = ac; // bool
        this.isCurrentItem = ic; // bool
        this.type = tp;
    }
...
(function () {
    var _CameraCalc = function () {
        ...
        this.SensorWidth = 6.17;
        this.SensorHeight = 4.55;
        this.ImageWidth = 4000;
        this.ImageHeight = 3000;
        this.LandScape = true;
        this.FixedOrientation = false;
        this.MinTriggerInterval = 1.0;
        this.Name = "";
        this.ValueSetIsDistance = true;
        this.DistanceToSurface = 10;
        this.ImageDensity = 25;
        this.AdjustedFootprintSide = 25;
        this.AdjustedFootprintFrontal = 25;
        this.TriggerInTurnAround = true;
    }

    _CameraCalc.prototype.recalcTriggerDistance = function()
    {
        do {
            this.DisableRecalc = true;
            var focalLength = this.focalLength;
            var sensorWidth = this.sensorWidth;
            var sensorHeight = this.sensorHeight;
            var imageWidth = this.imageWidth;
            var imageHeight = this.imageHeight;
            var imageDensity = this.ImageDensity;

            if (focalLength <= 0 || sensorWidth <= 0 || sensorHeight <= 0 || imageWidth <= 0 || imageHeight <= 0 || imageDensity <= 0) {
                break;
            }

            if (this.ValueSetIsDistance) {
                this.imageDensity = (this.DistanceToSurface * sensorWidth * 100.0) / (imageWidth * focalLength);
            } else {
                this.DistanceToSurface = (imageWidth * this.ImageDensity * focalLength) / (sensorWidth * 100.0);
            }

            imageDensity = this.imageDensity;

            if (this.LandScape) {
                this.ImageFootprintSide = (imageWidth * imageDensity) / 100.0;
                this.ImageFootprintFrontal = (imageHeight * imageDensity) / 100.0;
            } else {
                this.ImageFootprintSide = (imageHeight * imageDensity) / 100.0;
                this.ImageFootprintFrontal = (imageWidth * imageDensity) / 100.0;
            }
            this.AdjustedFootprintSide = this.ImageFootprintSide * ((100.0 - this.SideOverlap) / 100.0);
            this.AdjustedFootprintFrontal = this.ImageFootprintFrontal * ((100.0 - this.FrontalOverlap) / 100.0);
...
function offsetPolyline(line, distance) {
    ...
    if (line.points.length > 1) {
        ...
        for (var i = 1; i < rgOffsetEdges.length; i++) {
            if (rgOffsetEdges[i - 1].lineIntersect(rgOffsetEdges[i], null) == 0) {
                // Two lines are colinear
                newVertex = rgOffsetEdges[i].p2;
            }
            newVertex = rgOffsetEdges[i - 1].lineIntersect(rgOffsetEdges[i], 1);
